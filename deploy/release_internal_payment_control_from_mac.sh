#!/usr/bin/env bash
# Publish, review, merge, and deploy the non-fiscal internal payment controller.
# Run from any directory inside the Cellen checkout on the Mac:
#   bash deploy/release_internal_payment_control_from_mac.sh
set -Eeuo pipefail

BRANCH=${CELLEN_RELEASE_BRANCH:-feat/internal-payment-control}
PR_TITLE=${CELLEN_RELEASE_TITLE:-"feat: add non-fiscal internal payment control"}
...
git diff-tree --no-commit-id --name-only -r "$FEATURE_SHA" \
  | grep -Fx 'alembic/versions/0030_internal_payment_controls.py' >/dev/null || {
    echo "The release commit does not contain migration 0030." >&2
    exit 2
  }
...
if [[ -z $PR_URL ]]; then
  PR_URL=$(gh pr create \
    ...
    --head "$BRANCH" \
    --title "$PR_TITLE" \
    --body "Adds a non-fiscal payment controller for schools without active Finreg. Matrícula fees create internal charges automatically; staff can also create charges from billable items. Parents see charges for their linked children and upload proof, then authorized school finance staff confirm or reject it. Finreg and internal controls are mutually exclusive.")
fi
...
echo
echo "Cellen internal payment control released successfully at $MERGED_SHA."
