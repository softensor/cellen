...
composition=$(sql finreg "SELECT COALESCE(vertical_profile, 'generic_service')||'|'||encode(convert_to(COALESCE(enabled_capabilities, '[]'::jsonb)::text, 'UTF8'), 'hex')||'|'||CASE WHEN upper(country) IN ('AO', 'AGO') THEN 'angola' ELSE '' END FROM companies WHERE id='$company_id';" 2>/dev/null || true)
IFS='|' read -r composition_profile composition_capabilities_hex composition_country_pack <<<"$composition"
fingerprint=$(cd "$FINREG_DIR/backend" && set -a && source /etc/finreg.env && set +a && export PYTHONPATH="$FINREG_DIR/backend" && "$FINREG_DIR/.venv/bin/python" -c 'import json, sys; from app.core.module_registry import module_registry; capabilities = json.loads(bytes.fromhex(sys.argv[2]).decode()); print(module_registry.resolve(sys.argv[1], capabilities, sys.argv[3] or None).fingerprint)' "$composition_profile" "$composition_capabilities_hex" "$composition_country_pack" 2>/dev/null || true)
...
for scope in documents:read documents:write payments:write receipts:read receipts:write reports:read billing_plans:read billing_plans:write products:read products:write employees:write workspace:launch; do
